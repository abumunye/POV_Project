...
entity LED_Strip_Control is
    ...
    port (
        ...
        --Pixel Data
        i_Pixel_Data : in t_Vector_Array(0 to 72*3);
    ...
    begin
        if i_Rst_H = '1' then
            ...
            case r_STATE is
                when START_FRAME =>
                    r_TX_DV <= '0';
                    if r_TX_Count < 4 then -- Send 4 0x00 bytes for first frame
                ...
                when PIXEL_RGB =>
                    ...
                    if r_TX_Count < (g_NUMBER_OF_LEDs + 4) then
                        if w_TX_Ready ='1' then
                            case r_LED_STATE is
                                when GLOBAL =>
                                    r_TX_Byte <= (others => '1'); -- Send 0xFF as brightness. If brightness is lowered using this byte you significantly lower the update frequnecy
                                    r_LED_STATE <= BLUE;   
                                when BLUE =>
                    ...
                    else -- After finishing sending all pixel data
                        r_STATE <= END_FRAME;
                    end if;
                when END_FRAME =>
                    r_TX_DV <= '0';
